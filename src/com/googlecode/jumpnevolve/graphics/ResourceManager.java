...
public class ResourceManager {
	 ...
	 * 
	 * @param identifier Pfad zur Ressource.
	 */
	public void schedule(String identifier) {
		identifier = normalizeIdentifier(identifier);
		if(!this.schedule.contains(identifier) && !this.images.containsKey(identifier) && !this.sounds.containsKey(identifier)) {
	...
	private void load(String identifier) {
		try {
			...
			} else if(identifier.endsWith(".png?reverse")) { // Umgedrehted Bild laden
				this.images.put(identifier, getImage(identifier.substring(0, identifier.indexOf('?'))).getFlippedCopy(true, false));
